...
function checkWord(selected) {
    ...
    // golim variantele de răspuns după alegere (ALESIA)
    if (optionsContainer) {
        optionsContainer.innerHTML = "";
    }

    // găsim obiectul cuvântului ales (ALESIA)
    const chosenWord = words.find(w => w.word === selected) || null;

    // verificare corectitudine (Ciprian)
    const isCorrect = (selected === currentWord.word);

    // progres global (Ciprian)
    updateGlobalProgress(isCorrect, `WORD:${currentWord.word}`);

    if (isCorrect) {
        // răspuns corect (ALESIA + ciprian)
...
function validateColorAnswer(answerRaw, chosenColor = null) {
    ...
    // 🔵 CIPRIAN – actualizăm progresul global
    updateGlobalProgress(isCorrect, `COLOR:${currentColor.name}`);

    // 🔵 ALESIA – ascunde opțiunile și butonul de voce după răspuns
    if (optionsContainer) {
        optionsContainer.innerHTML = "";
    }
    if (voiceBtn) {
    ...
    if (isCorrect) {
        ...
        // ❌ RĂSPUNS GREȘIT – ALESIA + adaptare Ciprian
        fb.className = "feedback error";
        ...
        speak(`Nu este corect. Ai ales ${chosenNameText}, iar culoarea corectă este ${currentColor.sound}.`);
...
window.addEventListener("load", () => {
    ...
    // CIPRIAN – Inițializăm bara globală de progres
    const fill = document.getElementById("global-progress-fill");
